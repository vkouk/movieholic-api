...
export const getLatestOrders = async (req, res) => {
  const latestOrders = await Rental.find()
    .sort("-dateOrdered")
    .populate("customer")
    .populate("movies")
    .populate("series");
...
export const mostRented = async (req, res) => {
  const allOrders = await Rental.find()
    .sort("-dateOrdered")
    .populate("movies")
    .populate("series");
  let allOrderMovies = [];
  ...
  res.json([
    ...sortObject(countMostRentedData(allOrderMovies)),
    ...sortObject(countMostRentedData(allOrderSeries))
  ]);
};
